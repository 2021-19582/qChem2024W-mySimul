...
# 1-1] Define molecule
import FeScu
m_FeScu = FeScu.ActiveSpaceModel(model)
mol = m_FeScu.gen_mol()
...
if not os.path.isfile(dumpfile):
    ...
    mf = m_FeScu.do_mf(mol, chkfile=dumpfile_mf)
...
m_FeScu.init_model(dumpfile, method=method, dbg=dbg)
h1e, g2e, ecore = m_FeScu.gen_ham(tol=1e-6)
hso, hso2e = m_FeScu.gen_hso(somf=somf) # hso2e is None if somf is True
hr = m_FeScu.gen_hr()

n_mo, n_elec, n_core = m_FeScu.norb, m_FeScu.n_elec, m_FeScu.n_core
n_core, n_inactive, n_external, n_active = m_FeScu.n_core, m_FeScu.n_inactive, m_FeScu.n_external, m_FeScu.n_active
