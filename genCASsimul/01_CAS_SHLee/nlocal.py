...
def dumpLUNO(fname,thresh=0.05):
   ...
   # Spin-averaged DM
   pTa = numpy.dot(ma[:,:int(nalpha)],ma[:,:int(nalpha)].T) # FIXME
   pTb = numpy.dot(mb[:,:int(nbeta)],mb[:,:int(nbeta)].T) # FIXME
   pT = 0.5*(pTa+pTb)
   # Lowdin basis
   s12 = sqrtm(ova)
   s12inv = lowdin(ova)
   ...
   cOrbs_frz = coeff[:,0:1]
   cOrbs = coeff[:,1:actIndices[0]]
   aOrbs = coeff[:,actIndices]
   vOrbs = coeff[:,actIndices[-1]+1:]
   nb = cOrbs.shape[0]
   nc_frz = cOrbs_frz.shape[1]
   nc = cOrbs.shape[1]
   na = aOrbs.shape[1]
   nv = vOrbs.shape[1]
   ...
   # loc Boys
   clmo = lo.Boys(mol, clmo).kernel()
   almo = lo.Boys(mol, almo).kernel()
   ...
   # P-SORT
   mo_c_f,n_c_f,e_c_f = ulocal.psort(ova,fav,pT,clmo_frz)
   ...
   lmo = numpy.hstack((clmo_frz, mo_c,mo_o,mo_v)).copy()
   enorb = numpy.hstack([e_c_f, e_c,e_o,e_v])
   occ = numpy.hstack([n_c_f, n_c,n_o,n_v])
   # CHECK
